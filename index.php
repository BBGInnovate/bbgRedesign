...
$posts_query_params = array(
	'post_type' => 'post',
	'paged' => $paged
);
$news_posts_query = new WP_Query($posts_query_params);
$all_news_posts = $news_posts_query->posts;
...
<main id="main" role="main">
	...
	if ((!is_paged())) {
		...
		echo 	build_main_head_article($all_news_posts[0]);
		echo '</div>';
		array_shift($all_news_posts);
	}

	echo '<div class="archive-column">';
	foreach($all_news_posts as $cur_article) {
		echo build_horizontal_image_title_only_article($cur_article);
	}
	echo '</div>';
	...
	<?php
		// PAGINATION DOES NOT WORK THIS WAY
		// if (have_posts()) :
		// 	$counter = 0;
