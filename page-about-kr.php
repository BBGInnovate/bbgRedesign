...
function showUmbrellaArea($atts) {	
	...
	if ($columnType == "file") {
		...
		$linkSuffix  = ' <span class="bbg__file-size">(';
		$linkSuffix .= 		$fileExt;
		$linkSuffix .= 		', ';
		$linkSuffix .= 		$fileSize;
		$linkSuffix .= ')</span>';
	...
	if ($layout == 'full') {
		// Output variables
		$flex_row_article  = '<article class="';
		$flex_row_article .= 	$gridClass;
		$flex_row_article .= 	' bbg__about__grandchild bbg__about__child">';

		if ($columnTitle == "") {
			if ($forceContentLabels) {
				$flex_row_article .= '<h6 class="bbg__label">&nbsp;</h6>';	
			}
		} else {
			if ($link != "") {
				$columnTitle = '<a ' . $anchorTarget . ' href="' . $link . '">' . $columnTitle . '</a>';
			}
			$flex_row_article .= '<h6 class="bbg__label">' . $columnTitle . '</h6>';
		}
		if ($thumbSrc) {
			$flex_row_article .= '<div class="single-post-thumbnail clear bbg__article-header__thumbnail--medium">';
			$flex_row_article .= '<a ' . $anchorTarget . ' href="' . $link . '" rel="bookmark" tabindex="-1"><img width="1040" height="624" src="' . $thumbSrc .  '" class="attachment-large-thumb size-large-thumb"></a>';
			$flex_row_article .= '</div>';	
		}
		
		$flex_row_article .= '<h3 class="bbg__about__grandchild__title"><a ' . $anchorTarget . ' href="' . $link . '">' . $itemTitle . '</a>'  . $linkSuffix . '</h3>';
		if ($subTitle != "") {
			$flex_row_article .= '<h5 class="bbg__about__grandchild__subtitle">' . $subTitle . '</h5>';
		}
		$flex_row_article .= $description; // Output page excerpt
		$flex_row_article .= '</article>';
		echo $flex_row_article;
	} else {

		$flex_row_article .= '<article class="' . $gridClass . ' bbg__about__grandchild">';
		$columnTitle = $itemTitle;
		if ($link != "") {
			$columnTitle = '<a '  . $anchorTarget . ' href="' . $link . '">' . $columnTitle . '</a>';
		}
		$columnTitle = $columnTitle . $linkSuffix;
		$flex_row_article .= '<h3 class="bbg__about__grandchild__title">' . $columnTitle . '</h3>';	
		$flex_row_article .= '<a '  . $anchorTarget . ' href="' . $link . '">';
		$flex_row_article .= '<div class="bbg__about__grandchild__thumb" style="background-image: url(' . $thumbSrc . '); background-position:center center;"></div></a>' . $description;
		$flex_row_article .= '</article>';
		echo $flex_row_article;
	}
}
